...
void IntHandlerDrvTmrInstance1(void) // 10us timer, controls US sensors
{
    ...
    {   
        if (SENSORSTATE == FRONTTRIG || SENSORSTATE == FRONTCAP)
            SENSORSTATE = RIGHTTRIG;
        else if (SENSORSTATE == RIGHTTRIG || SENSORSTATE == RIGHTCAP)
            SENSORSTATE = LEFTTRIG;
        else if (SENSORSTATE == LEFTTRIG || SENSORSTATE == LEFTCAP)
    ...
    switch(SENSORSTATE) 
    {
        ...
        case RIGHTTRIG:
        {
            PLIB_PORTS_PinWrite(PORTS_ID_0, TRIGGER_CHANNEL_RIGHT, TRIGGER_BIT_RIGHT, 1);
            DRV_TMR1_CounterClear();
            resetUSCount('a');
            SENSORSTATE = RIGHTCAP;
            break;
        }
        case RIGHTCAP:
        {
            PLIB_PORTS_PinWrite(PORTS_ID_0, TRIGGER_CHANNEL_RIGHT, TRIGGER_BIT_RIGHT, 0);
            if(PLIB_PORTS_PinGet(PORTS_ID_0, ECHO_CHANNEL_RIGHT, ECHO_BIT_RIGHT) == 1)
                incUSCount('r');
            else if(PLIB_PORTS_PinGet(PORTS_ID_0, ECHO_CHANNEL_RIGHT, ECHO_BIT_RIGHT) == 0 && getUSCount('r') > 0)
            {
                float holder;
                holder = ((getUSCount('r')* 1e-5) * 171.5) * 1000;
                if (holder < 950) {
                    setTx('r', (tempR + holder) / 2);
                    tempR = holder;
                }
                SENSORSTATE = LEFTTRIG;
            }
        ...
        case LEFTTRIG:
        {
            PLIB_PORTS_PinWrite(PORTS_ID_0, TRIGGER_CHANNEL_LEFT, TRIGGER_BIT_LEFT, 1);
            DRV_TMR1_CounterClear();
            resetUSCount('a');
            SENSORSTATE = LEFTCAP;
            break;
        ...
        case LEFTCAP:
        {
            PLIB_PORTS_PinWrite(PORTS_ID_0, TRIGGER_CHANNEL_LEFT, TRIGGER_BIT_LEFT, 0);
            if(PLIB_PORTS_PinGet(PORTS_ID_0, ECHO_CHANNEL_LEFT, ECHO_BIT_LEFT) == 1)
                incUSCount('l');
            else if(PLIB_PORTS_PinGet(PORTS_ID_0, ECHO_CHANNEL_LEFT, ECHO_BIT_LEFT) == 0 && getUSCount('l') > 0)
            {
                float holder;
                holder = ((getUSCount('l')* 1e-5) * 171.5) * 1000;
                if (holder < 950) {
                    setTx('l', (tempL + holder) / 2);
                    tempL = holder;
                }
                SENSORSTATE = FRONTTRIG;
            }
