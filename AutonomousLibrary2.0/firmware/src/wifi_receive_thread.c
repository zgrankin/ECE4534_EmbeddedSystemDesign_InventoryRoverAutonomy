...
//    {
    ...
    else if (strstr(message, "Destination"))
    {
        Message_Nav tx;
    ...
    else if (strstr(message, "map"))
    {
        ...
        tx.type = 1;
        tx.map0.coordX = mapParse(message, result, 'x', "map0");
        tx.map0.coordY = mapParse(message, result, 'y', "map0");
        tx.map0.conf = mapParse(message, result, 'c', "map0");
        
        tx.map1.coordX = mapParse(message, result, 'x', "map1");
        tx.map1.coordY = mapParse(message, result, 'y', "map1");
        tx.map1.conf = mapParse(message, result, 'c', "map1");
        
        tx.map2.coordX = mapParse(message, result, 'x', "map2");
        tx.map2.coordY = mapParse(message, result, 'y', "map2");
        tx.map2.conf = mapParse(message, result, 'c', "map2");
        
        tx.map3.coordX = mapParse(message, result, 'x', "map3");
        tx.map3.coordY = mapParse(message, result, 'y', "map3");
        tx.map3.conf = mapParse(message, result, 'c', "map3");
        
        tx.map4.coordX = mapParse(message, result, 'x', "map4");
        tx.map4.coordY = mapParse(message, result, 'y', "map4");
        tx.map4.conf = mapParse(message, result, 'c', "map4");
        sendToQueueDetection(tx);
    }
}
...
int mapParse(char* message, char* result, char parseFor, char* whichMap)
{
    result = strstr(message, whichMap);
    result = &result[1];

    char temp[512];
    char* temp2;
    temp2 = parseFor == 'x' ? "xcoord" : 
            parseFor == 'y' ? "ycoord":
            parseFor == 'c' ? "conf" : 0;
    result = strstr(result, temp2);
    result = &result[1];
    ...
    while (*p) { // While there are more characters to process...
        if (isdigit(*p)) { // Upon finding a digit, ...
            int changeSign = (char)*(p-1) == '-' ? -1 : 1;
            val = strtol(p, &p, 10) * changeSign; // Read a number, ...
            break;
